...
const app=document.querySelector('#app'),home=document.querySelector('#homeButton'),hub=document.querySelector('#hubButton');let state={game:null,index:0,score:0,items:[],revealed:false,mistakes:[],reviewMode:false,reviewItems:[],reviewMistakes:[],reviewCorrect:0,roundTotal:0,initialScore:0};
...
function start(id){document.body.classList.add('playing');state.game=id;state.index=0;state.score=0;state.revealed=false;state.teams=[0,0];state.boardIntro=id==='bord';hub.hidden=true;home.hidden=false;if(id==='beeldwoord')state.items=sample(picturePairs,12);if(id==='betekenis')state.items=sample(words,10);if(id==='bornem')state.items=sample(places,8);if(id==='sorteren')state.items=sample(words.filter(x=>['kaart','landschap','bestuur'].includes(x.c)),10);if(id==='toets')state.items=sample(testQuestions,10).map(mixQuestion);if(id==='bord')state.items=shuffle([...boardQuestions,...landscapeQuestions.map(boardVersion),...serviceQuestions]).map(mixQuestion);if(id==='kaartvak')state.items=shuffle(mapLocations);if(id==='afvalladder')state.items=shuffle(wasteLadderSituations);if(id==='gebieden')state.items=shuffle(areaQuestions);if(id==='landschappen')state.items=shuffle(landscapeQuestions).map(mixQuestion);if(id==='diensten')state.items=shuffle(serviceQuestions).map(mixQuestion);render()}
function render(){window.scrollTo({top:0,left:0,behavior:'auto'});if(state.game==='bord'&&state.boardIntro)return boardIntro();if(state.index>=state.items.length)return finishRound();({beeldwoord:pictureMatch,betekenis:meaning,bornem:bornem,sorteren:sort,toets:test,bord:board,kaartvak:mapGridExercise,afvalladder:wasteLadderGame,gebieden:areaGame,landschappen:landscapeGame,diensten:serviceGame}[state.game])();addPromptAudio();addAnswerAudio()}
function rememberMistake(){const item=state.items[state.index],list=state.reviewMode?state.reviewMistakes:state.mistakes;if(item&&!list.includes(item))list.push(item)}
function finishRound(){if(!state.reviewMode){state.roundTotal=state.items.length;state.initialScore=state.score}const retry=state.reviewMode?state.reviewMistakes:state.mistakes;if(retry.length){state.reviewMode=true;if(!state.reviewItems.length)state.reviewItems=[...retry];state.items=shuffle([...retry]);state.reviewMistakes=[];state.index=0;app.innerHTML=`<section class="result-card"><div class="result-emoji">💪</div><p class="eyebrow">FOUTENRONDE</p><h1>Nog even oefenen</h1><p>Je krijgt de vragen die fout waren opnieuw. Een vraag blijft terugkomen tot ze juist is.</p><button class="primary-button" id="reviewMistakes">Oefen mijn foutjes</button></section>`;document.querySelector('#reviewMistakes').onclick=render;sayText('Nog even oefenen. Je krijgt de vragen die fout waren opnieuw.');return}state.score=state.initialScore;state.items=new Array(state.roundTotal);result()}
const mistakeObserver=new MutationObserver(changes=>{if(changes.some(change=>change.target.classList?.contains('wrong')||change.target.classList?.contains('wrong-shake')))rememberMistake()});mistakeObserver.observe(app,{subtree:true,attributes:true,attributeFilter:['class']});
...
const boardRound=board;board=function(){boardRound();const controls=document.querySelector('.board-controls');if(!controls)return;const difficult=document.createElement('button');difficult.className='secondary-button';difficult.textContent='Deze vraag nog eens oefenen';difficult.onclick=()=>{rememberMistake();difficult.textContent='Komt terug in de foutenronde ✓';difficult.disabled=true};controls.prepend(difficult)};
const startRound=start;start=function(id){state.mistakes=[];state.reviewMode=false;state.reviewItems=[];state.reviewMistakes=[];state.reviewCorrect=0;state.roundTotal=0;state.initialScore=0;return startRound(id)};
home.onclick=menu;
