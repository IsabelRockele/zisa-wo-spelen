...
home.onclick=menu;
const readButton=document.querySelector('#readButton');readButton.onclick=()=>{if(!('speechSynthesis'in window))return;window.speechSynthesis.cancel();const nodes=document.querySelectorAll('.game-card h3,.game-card p,.game-head h1,.question-card .prompt,.question-card .answer,.sort-choice,.board-question,.board-option,.result-card h1,.result-card p');const text=[...nodes].filter(n=>!n.hidden&&n.offsetParent!==null).map(n=>n.textContent.trim()).filter(Boolean).join('. ');const u=new SpeechSynthesisUtterance(text||'Kies een oefening.');u.lang='nl-BE';u.rate=.86;window.speechSynthesis.speak(u)};
