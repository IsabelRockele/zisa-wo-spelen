...
const boardExtraQuestions=[
 {kind:'Bornem herkennen',q:'Welke plaats in Bornem zie je?',img:'gemeentehuis-bornem.jpg',a:['het gemeentehuis','het station','Breeven'],right:0,why:'Dit is het gemeentehuis van Bornem.'},
 {kind:'Bornem herkennen',q:'Welke plaats in Bornem zie je?',img:'kasteel-bornem.jpg',a:['Kasteel van Bornem','Ter Dilft','de brandweer'],right:0,why:'Dit is Kasteel van Bornem.'},
 {kind:'Woord en beeld',q:'Welk woord past bij deze afbeelding?',img:'luchtfoto-school.png',a:['de luchtfoto','de legende','het adres'],right:0,why:'Je kijkt van bovenaf naar de omgeving: dat is een luchtfoto.'},
 {kind:'Kaart en rooster',q:'Het station staat in kolom C en rij 1. Wat is de vakcoördinaat?',icon:'🗺️',a:['C1','1C','B1'],right:0,why:'Eerst schrijf je de kolomletter en daarna het rijnummer: C1.'},
 {kind:'Kaart en rooster',q:'Het kasteel staat in kolom D en rij 4. Wat is de vakcoördinaat?',icon:'🏰',a:['D4','4D','C4'],right:0,why:'Eerst de letter van de kolom, dan het cijfer van de rij: D4.'},
 {kind:'Afval-ladder',q:'Een boek gaat naar de kringwinkel. Welke trede past?',icon:'📚',a:['hergebruik','verbranden','storten'],right:0,why:'Iemand anders kan het boek opnieuw gebruiken.'},
 {kind:'Afval-ladder',q:'Je neemt een eigen boodschappentas mee. Welke trede past?',icon:'🛍️',a:['voorkomen','recycleren','storten'],right:0,why:'Je voorkomt dat er een nieuwe wegwerpzak nodig is.'},
 {kind:'Afval-ladder',q:'Restafval gaat in een grote oven. Welke trede past?',icon:'🔥',a:['verbranden','hergebruik','voorkomen'],right:0,why:'Restafval wordt in grote ovens verbrand.'},
 {kind:'Gebieden',q:'Welk gebied zie je?',img:'gebied-woongebied-foto-v2.webp',a:['woongebied','industriegebied','natuurgebied'],right:0,why:'Je ziet vooral huizen en straten waar mensen wonen.'},
 {kind:'Gebieden',q:'Welk gebied zie je?',img:'gebied-landbouwgebied-foto-v2.webp',a:['landbouwgebied','woongebied','industriegebied'],right:0,why:'Je ziet akkers, landbouwmachines en open velden.'},
 {kind:'Gebieden',q:'Welk gebied zie je?',img:'gebied-natuurgebied-foto-v2.webp',a:['natuurgebied','woongebied','landbouwgebied'],right:0,why:'Hier krijgt de natuur veel ruimte en bescherming.'},
 {kind:'Gebieden',q:'Welk gebied zie je?',img:'gebied-industriegebied-foto-v2.webp',a:['industriegebied','natuurgebied','woongebied'],right:0,why:'Je ziet fabrieken, magazijnen en bedrijfswegen.'},
 {kind:'Woordenschat',q:'Wat betekent recycleren?',icon:'♻️',a:['Afval sorteren zodat er iets nieuws van wordt gemaakt','Alles bij het restafval gooien','Iets in de natuur achterlaten'],right:0,why:'Bij recycleren worden gebruikte materialen grondstof voor iets nieuws.'},
 {kind:'Bestuur',q:'Wie helpt de gemeente besturen?',icon:'🏛️',a:['de burgemeester en het bestuur','de gids en de boer','de treinbestuurder'],right:0,why:'De burgemeester werkt samen met het gemeentebestuur.'}
];
const boardQuestions=[...testQuestions,...boardExtraQuestions];
...
const serviceQuestions=[
 ...
 {q:'Er is een baby geboren. Waar wordt dat officieel opgeschreven?',icon:'👶',a:['bij de dienst burgerzaken','bij de sportdienst','bij de brandweer'],right:0,why:'De dienst burgerzaken registreert geboorten.'},
 {q:'Wie is de baas van de lokale politie en waakt mee over de veiligheid?',icon:'👮',a:['de burgemeester','de onthaalmedewerker','de apotheker'],right:0,why:'De burgemeester heeft belangrijke taken voor de veiligheid.'},
 {q:'Wie neemt de telefoon op en vertelt bij welke dienst je moet zijn?',icon:'☎️',a:['de onthaalmedewerker','de burgemeester','de politieagent'],right:0,why:'Het onthaal helpt bezoekers en verwijst hen naar de juiste dienst.'},
 {q:'Wie helpt bij vragen over afval, sorteren en het milieu?',icon:'♻️',a:['de milieudienst','de sportdienst','de politie'],right:0,why:'De milieudienst helpt bij vragen over afval en milieu.'},
 {q:'Je fiets is gestolen. Welke dienst heb je nodig?',icon:'🚲',a:['de politie','de brandweer','het openbaar vervoer'],right:0,why:'Een diefstal meld je bij de politie.'},
 {q:'Na hevige regen staat de kelder onder water. Welke dienst kan helpen?',icon:'🌧️',a:['de brandweer','de apotheek','de sportdienst'],right:0,why:'De brandweer helpt ook bij wateroverlast.'},
 {q:'Je wilt weten welke sporten je in Bornem kunt doen. Welke dienst helpt?',icon:'⚽',a:['de sportdienst','de milieudienst','de politie'],right:0,why:'De sportdienst geeft informatie over sportactiviteiten.'},
 {q:'Je gaat met de trein naar het werk. Welke dienst gebruik je?',icon:'🚆',a:['het openbaar vervoer','de brandweer','de milieudienst'],right:0,why:'Trein en bus horen bij het openbaar vervoer.'},
 {q:'Er ligt veel afval in het park. Bij welke dienst meld je dat?',icon:'🗑️',a:['de milieudienst','de sportdienst','de apotheek'],right:0,why:'De milieudienst behandelt vragen en meldingen over afval.'},
 {q:'Wie kan mensen officieel trouwen?',icon:'💍',a:['de burgemeester of een schepen','de onthaalmedewerker','de brandweer'],right:0,why:'De burgemeester of een bevoegde schepen kan een burgerlijk huwelijk sluiten.'},
 {q:'Waarvoor zorgt de gemeente in het verkeer?',icon:'🚸',a:['veilige fietspaden en zebrapaden','dat jouw fiets altijd hersteld is','dat iedereen een fietshelm draagt'],right:0,why:'De gemeente zorgt voor een veilige inrichting van de openbare weg.'}
];
const games=[
 ...
 {id:'landschappen',icon:'🌾',title:'Landschap en landbouw',text:'Herken Bornem als landelijk landschap en oefen de vier landbouwvormen.',img:'landschap-kasteel-bornem.webp'},
 {id:'diensten',icon:'🏛️',title:'Welke dienst helpt?',text:'Ontdek wie wat doet in de gemeente en wie je om hulp vraagt.',img:'gemeentehuis-bornem.jpg'}
 ,{id:'vindplek',icon:'👆',title:'Vind Bornem en onze school',text:'Duid Bornem aan op de provinciekaart en zoek De Linde op de luchtfoto.',img:'provincie-antwerpen-gemeenten.gif'}
 ,{id:'gemeentekaart',icon:'🧩',title:'Deelgemeenten en buurgemeenten',text:'Klik op de kaart welke plaatsen binnen Bornem liggen en welke gemeenten ernaast liggen.',img:'kaart-buurgemeente-bornem.svg'}
];
...
const boardVersion=x=>x.a.length<=3?x:{...x,a:[x.a[x.right],...sample(x.a.filter((_,i)=>i!==x.right),2)],right:0};
...
function start(id){document.body.classList.add('playing');state.game=id;state.index=0;state.score=0;state.revealed=false;state.teams=[0,0];state.boardIntro=id==='bord';hub.hidden=true;home.hidden=false;if(id==='beeldwoord')state.items=sample(picturePairs,12);if(id==='betekenis')state.items=sample(words,10);if(id==='bornem')state.items=sample(places,8);if(id==='sorteren')state.items=sample(words.filter(x=>['kaart','landschap','bestuur'].includes(x.c)),10);if(id==='toets')state.items=sample(testQuestions,10).map(mixQuestion);if(id==='bord')state.items=shuffle([...boardQuestions,...landscapeQuestions.map(boardVersion),...serviceQuestions]).map(mixQuestion);if(id==='kaartvak')state.items=shuffle(mapLocations);if(id==='afvalladder')state.items=shuffle(wasteLadderSituations);if(id==='gebieden')state.items=shuffle(areaQuestions);if(id==='landschappen')state.items=shuffle(landscapeQuestions).map(mixQuestion);if(id==='diensten')state.items=shuffle(serviceQuestions).map(mixQuestion);if(id==='vindplek')state.items=[{q:'Waar ligt Bornem? Klik Bornem aan op de kaart.',img:'provincie-antwerpen-gemeenten.gif',alt:'Lichte kaart van de provincie Antwerpen met de namen van de gemeenten',x1:.025,x2:.145,y1:.72,y2:.88,ok:'Juist! Bornem ligt linksonder, aan de grens van de provincie.'},{q:'Waar ligt onze school De Linde? Klik de school aan op de luchtfoto.',img:'luchtfoto-school.png',alt:'Luchtfoto van de omgeving van basisschool De Linde',x1:.24,x2:.43,y1:.30,y2:.50,ok:'Juist! Daar ligt basisschool De Linde.'}];if(id==='gemeentekaart')state.items=[{kind:'deel',q:'Klik de vier deelgemeenten van Bornem aan.',help:'Een deelgemeente ligt binnen de dikke grens van Bornem.'},{kind:'buur',q:'Klik de vier buurgemeenten aan.',help:'Een buurgemeente is een andere gemeente die rond Bornem ligt.'}];render()}
function render(){if('speechSynthesis'in window)window.speechSynthesis.cancel();window.scrollTo({top:0,left:0,behavior:'auto'});if(state.game==='bord'&&state.boardIntro)return boardIntro();if(state.index>=state.items.length)return result();({beeldwoord:pictureMatch,betekenis:meaning,bornem:bornem,sorteren:sort,toets:test,bord:board,kaartvak:mapGridExercise,afvalladder:wasteLadderGame,gebieden:areaGame,landschappen:landscapeGame,diensten:serviceGame,vindplek:findPlaceGame,gemeentekaart:municipalityMapGame}[state.game])();addPromptAudio();addAnswerAudio()}
...
function landscapeGame(){const x=state.items[state.index];app.innerHTML=head('Landschap en landbouw',state.items.length)+`<section class="question-card landscape-question">${x.img?`<img class="landscape-photo" src="assets/images/${x.img}?v=1" alt="Realistische oefenfoto">`:`<div class="landscape-icon" aria-hidden="true">${x.icon}</div>`}<p class="prompt">${esc(x.q)}</p><div class="answer-grid">${x.a.map((o,i)=>`<button class="answer" data-right="${i===x.right}">${esc(o)}</button>`).join('')}</div><p class="feedback" id="feedback"></p></section>`;bindAnswers()}
function serviceGame(){const x=state.items[state.index];app.innerHTML=head('Welke dienst helpt?',state.items.length)+`<section class="question-card service-question"><div class="service-icon" aria-hidden="true">${x.icon}</div><p class="prompt">${esc(x.q)}</p><div class="answer-grid">${x.a.map((o,i)=>`<button class="answer" data-right="${i===x.right}">${esc(o)}</button>`).join('')}</div><p class="feedback" id="feedback"></p></section>`;document.querySelectorAll('.answer').forEach(b=>b.onclick=()=>{if(document.querySelector('.answer.correct,.answer.wrong'))return;const right=b.dataset.right==='true';b.classList.add(right?'correct':'wrong');if(!right)document.querySelector('.answer[data-right="true"]').classList.add('correct');if(right)state.score++;document.querySelector('#feedback').innerHTML=`${right?'Goed gekozen!':'Kijk naar het groene antwoord.'} ${esc(x.why)} <button class="next-button" id="next">Volgende</button>`;document.querySelector('#next').onclick=()=>{state.index++;render()}})}
...
function findPlaceGame(){const x=state.items[state.index],source=x.img.includes('provincie-')?'<small class="map-source">Kaart: d-maps.com — bron: https://www.d-maps.com/carte.php?lang=en&amp;num_car=35380</small>':'';app.innerHTML=head('Vind je plek',state.items.length)+`<section class="question-card find-place-question"><p class="prompt">${esc(x.q)}</p><button class="click-map" id="clickMap" type="button" aria-label="${esc(x.q)}"><img src="assets/images/${x.img}" alt="${esc(x.alt)}"><span class="click-mark" id="clickMark" hidden></span></button>${source}<p class="find-tip">Tik of klik rechtstreeks op de juiste plaats in de afbeelding.</p><p class="feedback" id="feedback"></p></section>`;const map=document.querySelector('#clickMap'),mark=document.querySelector('#clickMark'),feedback=document.querySelector('#feedback');map.onclick=e=>{if(map.classList.contains('solved'))return;const r=map.getBoundingClientRect(),px=(e.clientX-r.left)/r.width,py=(e.clientY-r.top)/r.height,right=px>=x.x1&&px<=x.x2&&py>=x.y1&&py<=x.y2;mark.hidden=false;mark.style.left=`${px*100}%`;mark.style.top=`${py*100}%`;mark.className=`click-mark ${right?'right':'wrong'}`;if(!right){feedback.textContent='Nog niet. Kijk goed naar de namen en probeer opnieuw.';sayText('Nog niet. Kijk goed en probeer opnieuw.');return}map.classList.add('solved');state.score++;feedback.innerHTML=`${esc(x.ok)} <button class="next-button" id="next">${state.index+1===state.items.length?'Bekijk mijn resultaat':'Volgende opdracht'}</button>`;sayText(x.ok);document.querySelector('#next').onclick=()=>{state.index++;render()}}}
function municipalityMapGame(){const x=state.items[state.index],parts=['Bornem','Hingene','Mariekerke','Weert'],neighbours=['Temse','Hamme','Puurs-Sint-Amands','Willebroek'];app.innerHTML=head('Gemeenten op de kaart',state.items.length)+`<section class="question-card municipality-question"><p class="prompt">${esc(x.q)}</p><div class="municipality-board" id="municipalityBoard"><button class="municipality outer north" data-kind="buur">Temse</button><button class="municipality outer east" data-kind="buur">Hamme</button><button class="municipality outer south" data-kind="buur">Puurs-<br>Sint-Amands</button><button class="municipality outer west" data-kind="buur">Willebroek</button><div class="bornem-core" aria-label="Bornem met zijn vier deelgemeenten">${parts.map((name,i)=>`<button class="municipality part part-${i+1}" data-kind="deel">${name}</button>`).join('')}</div></div><div class="municipality-actions"><button class="help-button" id="mapHelp" type="button">💡 Hulp nodig?</button><span id="mapHelpText"></span></div><p class="feedback" id="feedback"></p></section>`;const board=document.querySelector('#municipalityBoard'),feedback=document.querySelector('#feedback'),targets=[...board.querySelectorAll(`[data-kind="${x.kind}"]`)];board.querySelectorAll('.municipality').forEach(button=>button.onclick=()=>{if(button.classList.contains('chosen')||board.classList.contains('solved'))return;if(button.dataset.kind!==x.kind){button.classList.add('wrong');feedback.textContent=x.kind==='deel'?'Die plaats ligt niet binnen Bornem. Probeer opnieuw.':'Die plaats is een deel van Bornem, geen andere gemeente ernaast.';sayText(feedback.textContent);setTimeout(()=>button.classList.remove('wrong'),550);return}button.classList.add('chosen');if(targets.every(target=>target.classList.contains('chosen'))){board.classList.add('solved');state.score++;feedback.innerHTML=`Goed! Je hebt ze allemaal gevonden. <button class="next-button" id="next">${state.index+1===state.items.length?'Bekijk mijn resultaat':'Volgende opdracht'}</button>`;sayText('Goed! Je hebt ze allemaal gevonden.');document.querySelector('#next').onclick=()=>{state.index++;render()}}});document.querySelector('#mapHelp').onclick=()=>{document.querySelector('#mapHelpText').textContent=x.help;targets.forEach(target=>target.classList.add('hint-target'));sayText(x.help)}}
...
function boardIntro(){app.innerHTML=`<section class="board-shell board-welcome"><div class="result-emoji">🌟</div><p class="eyebrow">SAMEN AAN HET BORD</p><h1>Sterrenronde</h1><p class="board-question">Zo spelen we</p><ol class="board-steps"><li>Iedereen krijgt een <b>rode</b>, <b>blauwe</b> en <b>gele</b> kleurkaart.</li><li>Luister naar de vraag en bekijk de drie antwoorden.</li><li>Steek de kleur van jouw antwoord tegelijk omhoog.</li><li>De juf toont daarna het juiste antwoord.</li></ol><p class="board-mix">In de ronde komen woordenschat, Bornem, kaarten, gebieden, bestuur en afval aan bod.</p><button class="primary-button" id="startBoard">Start de sterrenronde</button></section>`;document.querySelector('#startBoard').onclick=()=>{state.boardIntro=false;sayText('Klaar voor de sterrenronde. Kies bij elke vraag rood, blauw of geel en steek jouw kleurkaart omhoog.');render()}}
function board(){const x=state.items[state.index],colors=['rood','blauw','geel'];app.innerHTML=`<section class="board-shell"><div class="board-top"><span>${esc(x.kind||'Thema 1')} · Vraag ${state.index+1} van ${state.items.length}</span><div class="team-score"><button data-team="0">Team zon: ${state.teams[0]}</button><button data-team="1">Team maan: ${state.teams[1]}</button></div></div><p class="board-instruction">Steek rood, blauw of geel omhoog.</p><p class="board-question">${esc(x.q)}</p>${x.img?`<img class="board-image" src="assets/images/${x.img}?v=2" alt="Afbeelding bij de vraag">`:x.icon?`<div class="board-icon" aria-hidden="true">${x.icon}</div>`:''}<div class="board-options">${x.a.slice(0,3).map((o,i)=>`<div class="board-option" data-right="${i===x.right}"><b>${colors[i]}</b>${esc(o)}</div>`).join('')}</div><div class="board-explain" id="boardExplain" hidden><strong>Juiste kleur: ${colors[x.right]}</strong><span>${esc(x.why||'Kijk goed naar de kenmerken op de foto en bij de omschrijving.')}</span></div><div class="board-controls"><button class="secondary-button" id="revealBoard">Toon het antwoord</button><button class="primary-button" id="nextBoard" hidden>Volgende vraag</button></div></section>`;document.querySelectorAll('[data-team]').forEach(b=>b.onclick=()=>{state.teams[Number(b.dataset.team)]++;board();addPromptAudio();addAnswerAudio()});document.querySelector('#revealBoard').onclick=()=>{document.querySelectorAll('.board-option').forEach(o=>o.classList.add(o.dataset.right==='true'?'board-correct':'board-dim'));document.querySelector('#boardExplain').hidden=false;document.querySelector('#nextBoard').hidden=false;document.querySelector('#revealBoard').hidden=true};document.querySelector('#nextBoard').onclick=()=>{state.index++;render()}}
