...
const games=[
 ...
 {id:'wonen',icon:'🏡',title:'Wonen en landschap',text:'Herken flats, rijhuizen, vrijstaande woningen en landschappen.',img:'woningen-samen-eigen.webp'},
 {id:'afval',icon:'♻️',title:'Sorteer het afval',text:'Kies voor elk voorwerp de juiste afvalbak.',img:'afval-illustratie.jpg'}
];
...
function start(id){state.game=id;state.index=0;state.score=0;state.revealed=false;state.teams=[0,0];hub.hidden=true;home.hidden=false;if(id==='beeldwoord')state.items=sample(picturePairs,12);if(id==='betekenis')state.items=sample(words,10);if(id==='bornem')state.items=sample(places,8);if(id==='sorteren')state.items=sample(words.filter(x=>['kaart','landschap','bestuur'].includes(x.c)),10);if(id==='toets')state.items=sample(testQuestions,10).map(mixQuestion);if(id==='bord')state.items=shuffle(testQuestions).map(mixQuestion);if(id==='wonen')state.items=shuffle(housingQuestions);if(id==='afval')state.items=shuffle(wasteItems);render()}
function render(){if(state.index>=state.items.length)return result();({beeldwoord:pictureMatch,betekenis:meaning,bornem:bornem,sorteren:sort,toets:test,bord:board,wonen:housing,afval:waste}[state.game])();addAnswerAudio()}
function sayText(text){if(!('speechSynthesis'in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='nl-BE';u.rate=.86;window.speechSynthesis.speak(u)}
...
function waste(){const x=state.items[state.index];app.innerHTML=head('Sorteer het afval',state.items.length)+`<section class="question-card waste-card"><p class="class-tip">Op het bord? Laat iedereen de kleur van de juiste bak omhoogsteken.</p><div class="waste-object" aria-label="${esc(x.name)}"><span>${x.icon}</span><strong>${esc(x.name)}</strong></div><p class="prompt">In welke bak hoort dit?</p><div class="waste-grid">${wasteBins.map(bin=>`<button class="answer waste-bin bin-${bin.key}" data-right="${bin.key===x.bin}"><span class="bin-lid"></span><strong>${esc(bin.label)}</strong><small>${bin.color}</small></button>`).join('')}</div><p class="feedback" id="feedback"></p></section>`;bindAnswers()}
function board(){const x=state.items[state.index],colors=['rood','blauw','geel'];app.innerHTML=`<section class="board-shell"><div class="board-top"><span>Vraag ${state.index+1} van ${state.items.length}</span><div class="team-score"><button data-team="0">Team zon: ${state.teams[0]}</button><button data-team="1">Team maan: ${state.teams[1]}</button></div></div><p class="board-instruction">Kies rood, blauw of geel en steek jouw kleurkaart omhoog.</p><p class="board-question">${esc(x.q)}</p><div class="board-options">${x.a.map((o,i)=>`<div class="board-option"><b>${colors[i]}</b>${esc(o)}</div>`).join('')}</div><div class="board-explain" id="boardExplain" hidden><strong>Antwoord: ${colors[x.right]}</strong><span>${esc(x.why)}</span></div><div class="board-controls"><button class="secondary-button" id="revealBoard">Toon het antwoord</button><button class="primary-button" id="nextBoard" hidden>Volgende vraag</button></div></section>`;document.querySelectorAll('[data-team]').forEach(b=>b.onclick=()=>{state.teams[Number(b.dataset.team)]++;board();addAnswerAudio()});document.querySelector('#revealBoard').onclick=()=>{document.querySelector('#boardExplain').hidden=false;document.querySelector('#nextBoard').hidden=false;document.querySelector('#revealBoard').hidden=true};document.querySelector('#nextBoard').onclick=()=>{state.index++;render()}}
