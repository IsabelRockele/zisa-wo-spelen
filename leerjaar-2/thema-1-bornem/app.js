...
function render(){window.scrollTo({top:0,left:0,behavior:'auto'});if(state.index>=state.items.length)return result();({beeldwoord:pictureMatch,betekenis:meaning,bornem:bornem,sorteren:sort,toets:test,bord:board,kaartvak:mapGridExercise,afvalladder:wasteLadderGame,gebieden:areaGame}[state.game])();addPromptAudio();addAnswerAudio()}
